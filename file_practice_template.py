...
def main():
    ...
    print(f'The original list: {num_list}')
    total, average = calculate_sum_average(num_list)
    print(f"Sum = {total}, Average = {average:.2f}")
    largest_index = get_largest_idx(num_list)
    print(f'The largest number {num_list[largest_index]} is stored in index {largest_index}')
...
def get_largest_idx(numbers):
    """ find the index that holds the largeset numbers in the list """
    idx = 0
